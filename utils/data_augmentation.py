...
def construct_incremental_graph_preselected(dataset, edges, max_n_vertices, real_n_vertices, vertices_to_keep, exit_points, node_symbol, params, is_training_data, initial_idx=0, single_exit=False): # FI changed
    # avoid calculating this if it is just for generating new molecules for speeding up
    if params["generation"] and is_training_data: # FI changed
        return [], [], [], [], [], [], [], [], []
    # avoid the initial index is larger than real_n_vertices:
    ...
    # FI - Add edges between vertices in vertices_to_keep and update starting points
    # FI - TODO
    edges_to_ignore = []
    for src, edge_type, dst in edges:
    ...
    # FI - If single exit, only add one of the exit vectors to the queue
    if single_exit:
        exit_to_use = [random.choice(exit_points)]
    else:
        exit_to_use = exit_points
    # FI - Add vertices in vertices_to_keep to queue and update colour
    ...
    # FI - add initial idx to queue if no vertices to keep
    if len(queue) == 0:

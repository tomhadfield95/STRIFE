...
def compute_distance_and_angle_dataset(fragmentations, path_to_conformers, dataset, verbose=False, pdbbind_ids=[]):
...
#On some occasions, where there are explicit hydrogens, RemoveHs() doesn't work
#This function manually removes those Hs
def removeExplicitHs(mol):
    rwmol = Chem.RWMol(mol)
    toRemove = []
    for atom in mol.GetAtoms():
        if atom.GetAtomicNum() == 1 and atom.GetDegree() == 0:
            toRemove.append(atom.GetIdx())
...
def check_2d_filters_Elab(toks, pains_smarts, count=0, verbose=False):
    # Progress
    if verbose:
        if count % 1000 == 0:
            print("\rProcessed: %d" % count, end = '')
    
    # Input format: (Full Molecule (SMILES), Reference Elaboration (SMILES),Fragment (SMILES))
    ...
    #Remove explicit Hs (if any)
    clean_frag = removeExplicitHs(clean_frag)
...
def check_2d_filters_dataset(fragmentations, n_cores=1, pains_smarts_loc='data/wehi_pains.csv'):
    ...
    with Parallel(n_jobs=n_cores, backend='multiprocessing') as parallel:
        results = parallel(delayed(check_2d_filters_Elab)(toks, pains_smarts, count, True) for count, toks in enumerate(fragmentations))

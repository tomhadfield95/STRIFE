...
class DenseGGNNChemModel(ChemModel):
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.placeholders['abs_dist'] = tf.compat.v1.placeholder(tf.float32, [None, 5], name='abs_dist') # [b, 2]
        ...
        self.weights['node_combine_weights_in'] = tf.Variable(glorot_init([h_dim+6, h_dim+6]))
        self.weights['node_atten_weights_c_in'] = tf.Variable(glorot_init([h_dim+6, h_dim+6]))
        self.weights['node_atten_weights_y_in'] = tf.Variable(glorot_init([h_dim+6, h_dim+6]))

        # The weights for generating node symbol logits    
        self.weights['node_symbol_weights_in'] = tf.Variable(glorot_init([h_dim+6, self.params['num_symbols']]))
        ...
        for i in range(self.num_edge_types):
            self.weights['edge_type_%d_%s' % (i, direc)] = tf.Variable(glorot_init([feature_dimension+6, feature_dimension+6]))
            self.weights['edge_type_biases_%d_%s' % (i, direc)] = tf.Variable(np.zeros([1, feature_dimension+6]).astype(np.float32)) 
            self.weights['edge_type_output_%d_%s' % (i, direc)] = tf.Variable(glorot_init([feature_dimension+6, 1]))
        # weights for generating edge logits
        self.weights['edge_iteration_'+direc] = tf.Variable(glorot_init([feature_dimension+6, feature_dimension+6])) #was previously +3, change to +6 -TH
        self.weights['edge_iteration_biases_'+direc] = tf.Variable(np.zeros([1, feature_dimension+6]).astype(np.float32)) 
        self.weights['edge_iteration_output_'+direc] = tf.Variable(glorot_init([feature_dimension+6, 1]))            
    ...
    def generate_cross_entropy(self, idx, cross_entropy_losses, edge_predictions, edge_type_predictions, reverse=False):
        ...
        dist = tf.reshape(tf.tile(tf.reshape(self.placeholders['abs_dist'], [-1,1,5]), [1, v, 1]), [-1, 5])
    ...
    def construct_logit_matrices(self):
        ...
        dist = tf.tile(tf.reshape(self.placeholders['abs_dist'], [-1,1,5]), [1, v, 1]) # [b, v, 2]
        ...
        atten_c = tf.reshape(tf.matmul(tf.reshape(atten_c, [-1, h_dim+6]), self.weights['node_atten_weights_c_'+in_direc]), [-1, v, v, h_dim+6])
        atten_yi = tf.reshape(tf.matmul(tf.reshape(atten_yi, [-1, h_dim+6]), self.weights['node_atten_weights_y_'+in_direc]), [-1, v, v, h_dim+6])
        atten_mi = tf.nn.sigmoid(tf.add(atten_c, atten_yi) + atten_masks)
        ...
        z_sampled = z_sampled * self.ops['graph_state_mask_'+in_direc] +\
                    atten_mi * tf.reshape(tf.matmul(tf.reshape(z_sampled, [-1, h_dim+6]), self.weights['node_combine_weights_'+in_direc]), [-1, v, h_dim+6])

        # Logits for node symbols
        self.ops['node_symbol_logits_'+in_direc]=tf.reshape(tf.matmul(tf.reshape(z_sampled,[-1, h_dim+6]), self.weights['node_symbol_weights_'+in_direc]) + 

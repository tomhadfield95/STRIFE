...
class DenseGGNNChemModel(ChemModel):
    ...
    def default_params(cls):
        ...
        params.update({
                        ...
                        'train_file': 'data/molecules_train_single.json',
                        'valid_file': 'data/molecules_valid_single.json',
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.placeholders['abs_dist'] = tf.placeholder(tf.float32, [None, 26], name='abs_dist') # [b, 2]
        ...
        self.weights['node_combine_weights_in'] = tf.Variable(glorot_init([h_dim+27, h_dim+27]))
        self.weights['node_atten_weights_c_in'] = tf.Variable(glorot_init([h_dim+27, h_dim+27]))
        self.weights['node_atten_weights_y_in'] = tf.Variable(glorot_init([h_dim+27, h_dim+27]))

        # The weights for generating node symbol logits    
        self.weights['node_symbol_weights_in'] = tf.Variable(glorot_init([h_dim+27, self.params['num_symbols']]))
        ...
        for i in range(self.num_edge_types):
            self.weights['edge_type_%d_%s' % (i, direc)] = tf.Variable(glorot_init([feature_dimension+27, feature_dimension+27]))
            self.weights['edge_type_biases_%d_%s' % (i, direc)] = tf.Variable(np.zeros([1, feature_dimension+27]).astype(np.float32)) 
            self.weights['edge_type_output_%d_%s' % (i, direc)] = tf.Variable(glorot_init([feature_dimension+27, 1]))
        # weights for generating edge logits
        self.weights['edge_iteration_'+direc] = tf.Variable(glorot_init([feature_dimension+27, feature_dimension+27])) #was previously +3, change to +27 -TH
        self.weights['edge_iteration_biases_'+direc] = tf.Variable(np.zeros([1, feature_dimension+27]).astype(np.float32)) 
        self.weights['edge_iteration_output_'+direc] = tf.Variable(glorot_init([feature_dimension+27, 1]))            
    ...
    def generate_cross_entropy(self, idx, cross_entropy_losses, edge_predictions, edge_type_predictions, reverse=False):
        direc = "out"
        direc_r = "in"
        ...
        local_graph_repr_before_expansion = tf.reduce_sum(new_filtered_z_sampled, axis=1) /  \
                                            tf.reduce_sum(self.placeholders['node_mask_'+direc], axis=1, keep_dims=True) # [b, h + h] - FI ONLY NEED OUT
        ...
        global_graph_repr_before_expansion = tf.reduce_sum(filtered_z_sampled, axis=1) / \
                                            tf.reduce_sum(self.placeholders['node_mask_'+direc], axis=1, keep_dims=True) # FI ONLY NEED OUT
        ...
        dist = tf.reshape(tf.tile(tf.reshape(self.placeholders['abs_dist'], [-1,1,26]), [1, v, 1]), [-1, 26])
    ...
    def construct_logit_matrices(self):
        ...
        dist = tf.tile(tf.reshape(self.placeholders['abs_dist'], [-1,1,26]), [1, v, 1]) # [b, v, 2]
        ...
        atten_c = tf.reshape(tf.matmul(tf.reshape(atten_c, [-1, h_dim+27]), self.weights['node_atten_weights_c_'+in_direc]), [-1, v, v, h_dim+27])
        atten_yi = tf.reshape(tf.matmul(tf.reshape(atten_yi, [-1, h_dim+27]), self.weights['node_atten_weights_y_'+in_direc]), [-1, v, v, h_dim+27])
        atten_mi = tf.nn.sigmoid(tf.add(atten_c, atten_yi) + atten_masks)
        ...
        z_sampled = z_sampled * self.ops['graph_state_mask_'+in_direc] +\
                    atten_mi * tf.reshape(tf.matmul(tf.reshape(z_sampled, [-1, h_dim+27]), self.weights['node_combine_weights_'+in_direc]), [-1, v, h_dim+27])

        # Logits for node symbols
        self.ops['node_symbol_logits_'+in_direc]=tf.reshape(tf.matmul(tf.reshape(z_sampled,[-1, h_dim+27]), self.weights['node_symbol_weights_'+in_direc]) + 
    ...
    def calculate_incremental_results(self, raw_data, bucket_sizes, file_name, is_training_data): #  FI changed params
        incremental_results=[[], []]
        # copy the raw_data if more than 1 BFS path is added
        new_raw_data=[]
        for idx, d in enumerate(raw_data):
            # repeat twice - forward and back
            out_direc = "out"
            res_idx = 1
            ...
            for list_idx, starting_idx in enumerate(list_of_starting_idx):
                # choose a bucket
                chosen_bucket_idx = np.argmax(bucket_sizes > max(max([v for e in d['graph_out']   # FI - CHANGED BUCKETING SO THAT IN MAX(in, out) not just out
                                                                        for v in [e[0], e[2]]]),
                ...
                incremental_adj_mat,distance_to_others,node_sequence,edge_type_masks,edge_type_labels,local_stop, edge_masks, edge_labels, overlapped_edge_features=\
                        construct_incremental_graph_preselected(self.params['dataset'], edges_no_master, chosen_bucket_size,
                                            len(nodes_no_master), d['v_to_keep'], d['exit_points'], nodes_no_master, self.params, is_training_data, initial_idx=starting_idx) # FI still learn to construct from scratch
    ...
    def process_raw_graphs(self, raw_data, is_training_data, file_name, bucket_sizes=None):
        ...
        incremental_results, raw_data=self.calculate_incremental_results(raw_data, bucket_sizes, file_name, is_training_data) # FI changed
    ...
    def get_dynamic_feed_dict(self, elements, latent_node_symbol, incre_adj_mat, num_vertices, 
        ...
        return {
                self.placeholders['z_prior']: random_normal_states, # [1, v, out]
                self.placeholders['z_prior_in']: random_normal_states_in, # [1, v, h] - FI two noise vecs
    ...
    def search_and_generate_molecule(self, initial_idx, valences, 
        ...
        count_bonds = 0
        # Add in edges between vertices to keep
    ...
    def generate_graph_with_state(self, random_normal_states, random_normal_states_in, num_vertices,
        ...
        # Set keep vertices to 'correct' atom type
    ...
    def generate_new_graphs(self,data):
        ...
        file_name = '%s/%s_generated_smiles_%s.smi' % (self.params["output_dir"], self.run_id, self.params["dataset"])

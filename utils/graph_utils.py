...
def domain(num_dimensions):
    #sets up the domain for Bayesian Optimisation implemented by GPyOpt
    #Currently only sets up continuous domains over [-10,10]

    dom_list = [] #Initialise empty list 

    for i in range(num_dimensions):
        #Create dictionary and append to the list
        name = 'var_' + str(i+1)
        dictionary = {'name' : name, 'type' : 'continuous', 'domain' : (-10,10)}

        dom_list.append(dictionary)

    return(dom_list)


def set_up_PSO(num_dim):
    #sets up the domain for our particle swarm algorithm
    #As with BayesOpt it searches over continuous domains (-10,10)

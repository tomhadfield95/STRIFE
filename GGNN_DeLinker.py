...
class ChemModel(object):
    ...
    def run_epoch(self, epoch_name: str, epoch_num, data, is_training: bool):
        ...
        for step, batch_data in enumerate(batch_iterator):
            ...
            #----ADDED BY TH----#
            #Save model every 50k iterations within an epoch
            if step % 50000 == 0:
                #self.save_model(self.run_id + "_" + str(epoch_num)+ "_" + str(step) +("_%s.pickle" % (self.params["dataset"])))
                self.save_model(self.run_id + "_latest.pickle")
                #Save latest model details
                f = open(self.run_id + "_latest.txt", "w")
                f.write("The current saved model was step " + str(step) + "of epoch " + str(epoch_num))
                f.close()
